...
```{r}
p <- ggplot(mal_fem, aes(SettoreAttivitaEconomica,
...
```{r}
p <- ggplot(Fetal_econ_sect, aes(SettoreAttivitaEconomica,
...
```{r}
p <- ggplot(Non_Fetal_econ_sect, aes(SettoreAttivitaEconomica,
                          total, group=Year, colour=Year)) + 
     geom_line(size=1)
p <- p + geom_point()  +
       labs(title = "Number of incidents by economic sector", 
...
```{r}
p <- ggplot(Fetal_incidents, aes(Year,
                          total, group=Genere, colour=Genere)) + 
     geom_line(size=1)
p <- p + geom_point()  +
...
# province with condice
prov <- read.csv("map.csv")

# extract the number of acidentds
accidents <- Ita_Regions %>%
  count(LuogoAccadimento)

colnames(accidents) <- c("LuogoAccadimento", "total")

italy_map <- italy_map %>%
  dplyr::left_join(prov, by = "province")

italy_map_province <- italy_map %>%
  dplyr::left_join(accidents, by = "LuogoAccadimento")

```

```{r}
italy_map_province %>%
  mapview(zcol = "total", col.regions = viridisLite::turbo)
```
...
italy_map_region <- italy_map %>%
  group_by(region) %>%
  summarise(n = n())
df3 <- read.csv("Regions.csv")
map_region <- italy_map_region %>%
  left_join(df3, by = "region")
