...
ggplot(Mod_acc, 
  ...
  labs(title = "Participants by ModalitaAccadimento")
```
...
ggplot(Mod, 
  ...
  theme_void() +
  theme(legend.position = "FALSE") +
  labs(title = "Participants by ConSenzaMezzoTrasporto")
```
...
GradoMenomazione_60 <- select(Ita_Regions, 
                              GradoMenomazione, 
                              Indennizzo,
                              GiorniIndennizzati) %>%
          filter(GradoMenomazione > 60)
#head(GradoMenomazione_60) 
sapply(GradoMenomazione_60, function(x) length(unique(x)))
```
The table below shows information about the degree of impairment when it is more than 40:
```{r}
reactable(GradoMenomazione_60, searchable = TRUE, minRows = 10)
```

```{r}
prop.table(table(GradoMenomazione_60$Indennizzo))
```
...
GradoMenomazione_40 <- select(Ita_Regions, 
                              GradoMenomazione, 
                              Indennizzo,
                              GiorniIndennizzati) %>%
          filter(GradoMenomazione > 40)
#head(GradoMenomazione_40)
sapply(GradoMenomazione_40, function(x) length(unique(x)))
```
...
```
```{r}
prop.table(table(GradoMenomazione_40$Indennizzo))
```
